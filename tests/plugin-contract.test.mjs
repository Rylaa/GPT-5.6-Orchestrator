...
test('skill defines a Sol-main-session dynamic workflow', async () => {
  ...
  assert.match(skill, /novice-readable live agent cards/i)
  assert.match(skill, /high-contrast basic ANSI palette distinguishes Sol authority.*model families.*lifecycle states/is)
  assert.match(skill, /NO_COLOR=1.*GPT56_ORCHESTRATOR_COLOR=0/is)
  assert.match(skill, /assigned task, current step, elapsed time, recent work, and report state/i)
  assert.match(skill, /never exposes raw commands, command output, full prompts, credentials, or hidden reasoning/i)
  assert.match(skill, /first worker `spawn` automatically opens one detached right-side live-work panel/i)
  assert.match(skill, /concurrent and later workers appear as additional cards/i)
  assert.match(skill, /pane closes automatically when all workers in the run are terminal/i)
  assert.match(skill, /GPT56_ORCHESTRATOR_AUTO_PANE=0/)
  assert.match(skill, /already inside an attached tmux client/i)
...
test('README is concise and shows Sol controlling dynamic Codex subagents', async () => {
  ...
  assert.match(readme, /novice-readable live agent view/i)
  assert.match(readme, /Oh My Codex.*inspired.*basic ANSI palette/is)
  assert.match(readme, /Color\s+turns on automatically in tmux and interactive terminals/is)
  assert.match(readme, /compacts cards.*pane dimensions.*without redraw corruption/is)
  assert.match(readme, /automatic pane intentionally overrides.*NO_COLOR=1.*GPT56_ORCHESTRATOR_COLOR=0/is)
  assert.match(readme, /plugin-specific setting always wins/is)
  assert.match(readme, /OMX is not a dependency/i)
  assert.match(readme, /assigned task, current\s+step, elapsed time, recent work, and report state/i)
  assert.match(readme, /never prints raw\s+commands, command output, full prompts, credentials, or hidden reasoning/i)
  assert.match(readme, /first\s+worker `spawn`\s+automatically opens one detached right-side live-work panel/i)
  assert.match(readme, /concurrent and later spawns appear as additional cards/i)
  assert.match(readme, /pane closes automatically when\s+all workers in the run are terminal/i)
  assert.match(readme, /GPT56_ORCHESTRATOR_AUTO_PANE=0/)
  assert.match(readme, /already inside an attached tmux client/i)
  ...
  assert.match(readme, /tmux\s+is optional/i)
