...
test('manifest exposes the canonical GPT-5.6 Orchestrator identity', async () => {
  ...
  assert.match(manifest.version, /^0\.3\.1(?:\+codex\.\d{14})?$/)
...
test('skill defines a Sol-main-session dynamic workflow', async () => {
  ...
  assert.match(skill, /first worker `spawn` automatically opens one detached right-side dashboard/i)
  assert.match(skill, /concurrent and later spawns reuse it/i)
...
test('README is concise and shows Sol controlling dynamic Codex subagents', async () => {
  ...
  assert.match(readme, /first\s+worker `spawn` automatically opens one detached right-side dashboard/i)
  assert.match(readme, /concurrent and later spawns reuse it/i)
