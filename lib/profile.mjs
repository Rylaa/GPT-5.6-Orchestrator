...
export function resolveProfile({ model = '', override = 'auto' } = {}) {
  ...
  if (!['auto', 'sol', 'luna'].includes(normalizedOverride)) {
...
export function resolveThreshold({ profile, env = process.env }) {
  ...
  const globalOverride = parseThreshold(
    env.CODEX_SOL_FUSION_LEDGER_THRESHOLD,
    'Global ledger',
  )
  if (globalOverride !== null) return globalOverride

  const profileKey = `CODEX_SOL_FUSION_LEDGER_THRESHOLD_${profile.toUpperCase()}`
...
export function profileInstructionFile(profile) {
  if (profile === 'sol') return 'sol-chair.md'
  if (profile === 'luna') return 'luna-chair.md'
  throw new RangeError(`Unknown instruction profile: ${profile}`)
}
