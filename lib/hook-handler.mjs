...
const ROUTING_REMINDER = [
  ...
  'After clarification, select Q0 inline, Q1 Luna review, Q2 Terra review, or Q3 Terra plus Sol verification. Q1 defaults to standard; for deploy-only work on an already green remote SHA, Q1 deploy-fast may replace duplicate local tests and review with ordered predeploy, deploy, and smoke evidence. Use the controller evidence and close commands so ledger-backed completion has a current .workflow/closure.json.',
...
async function handleStop(payload, options) {
  ...
  const closure = await validateClosure({ cwd: payload.cwd, dataDir: options.dataDir })
  if (!closure.valid) {
    return blockCompletion(
      payload,
      options,
      `Do not claim completion until the risk-based QA closure is current and accepted. ${closure.reason}`,
      'invalid_qa_closure',
    )
  }
  return null
