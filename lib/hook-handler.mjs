...
const ROUTING_REMINDER = [
  ...
  'Read-only workers may run together. Write workers declare owned paths; the controller rejects overlap with an active writer.',
...
async function handlePolicyRehydration(payload, options, hookEventName) {
  ...
  return {
    hookSpecificOutput: {
      hookEventName,
      additionalContext: [CHAIR_REMINDERS[runtimeProfile], ROUTING_REMINDER].join(' '),
    },
  }
}
