...
export const MANAGED_AGENT_ROLES = Object.freeze({
  ...
  orchestrator_luna_reviewer: Object.freeze({
    model: 'gpt-5.6-luna',
    effort: 'high',
    sandbox: 'read-only',
    lane: 'narrow checklist review',
    instructions: `Review a narrow completed diff against explicit acceptance criteria only after the writer and direct tests finish. Stop and return to Sol if broad cross-file, security, or root-cause review is required. Do not edit files. ${COMMON_WORKER_RULES}`,
  }),
  ...
  orchestrator_terra_reviewer: Object.freeze({
    model: 'gpt-5.6-terra',
    effort: 'high',
    sandbox: 'read-only',
    lane: 'cross-file review',
    instructions: `Review broad or cross-file completed changes only after the writer and direct tests finish. Trace assumptions and edge cases, separate confirmed defects from inference, and escalate consequential conflict to Sol. Do not edit files. ${COMMON_WORKER_RULES}`,
  }),
  ...
  orchestrator_sol_verifier: Object.freeze({
    model: 'gpt-5.6-sol',
    effort: 'max',
    sandbox: 'read-only',
    lane: 'independent high-risk verification',
    instructions: `Independently verify a high-risk outcome from current artifacts. Check correctness, security boundaries, regression coverage, and open ledger items without treating worker narration as proof. Do not edit files. ${COMMON_WORKER_RULES}`,
  }),
})
