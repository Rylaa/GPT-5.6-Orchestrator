...
export function parseLedger(content) {
  ...
  for (const line of String(content).split(/\r?\n/)) {
    ...
    const idMatch = label.match(/^((?:[A-Za-z][A-Za-z0-9_-]*|\d+))[.)]?\s+/)
    const id = idMatch?.[1] ?? null
    const isDeferred = status === '~'
      && /^deferred\s*:/i.test(label)
      && /\buser-approved\b/i.test(label)
    if (status === 'x' || isDeferred) closedItems += 1
    else openItems.push(label)
    items.push({ id, label, status, closed: status === 'x' || isDeferred, deferred: isDeferred })
  }

  return { totalItems, closedItems, openItems, items }
}

export function ledgerItemIds(content) {
  return new Set(parseLedger(content).items.flatMap((item) => (
