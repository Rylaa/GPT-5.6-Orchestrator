...
export function parseLedger(content) {
  ...
  for (const line of String(content).split(/\r?\n/)) {
    const tierMatch = line.match(/^\s*QA-Tier:\s*(Q[0-3])\s*$/i)
    if (tierMatch) qaTier = tierMatch[1].toLowerCase()
    const profileMatch = line.match(/^\s*QA-Profile:\s*(standard|deploy-fast)\s*$/i)
    if (profileMatch) qaProfile = profileMatch[1].toLowerCase()
  ...
  return { totalItems, closedItems, openItems, qaTier, qaProfile }
}
