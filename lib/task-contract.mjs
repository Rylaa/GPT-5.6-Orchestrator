...
const MAX_TASK_OBJECTIVE_CHARS = 600
...
function normalizeTaskObjective(value, maxChars = MAX_TASK_OBJECTIVE_CHARS) {
...
export function extractTaskObjective(task, { maxChars = MAX_TASK_OBJECTIVE_CHARS } = {}) {
  const normalizedTask = String(task || '')
  const sections = parseKnownSections(normalizedTask, TASK_SECTION_NAMES, { rejectDuplicates: false })
  const sectionObjective = normalizeTaskObjective(sections.get('objective'), maxChars)
  if (sectionObjective) return sectionObjective
  const firstContentLine = normalizedTask
    .split(/\r?\n/)
    .map((line) => line.trim())
    .find((line) => line && !/^#{1,6}\s/.test(line))
  return normalizeTaskObjective(firstContentLine, maxChars)
}
...
export function validateTaskContract({ task, ledger = null, detailedThreshold = 1_500 }) {
  ...
  const objective = normalizeTaskObjective(sections.get('objective'))
    || extractTaskObjective(normalizedTask)
  if (!detailed) return { detailed: false, ledgerIds: [], objective }
  ...
  return { detailed: true, ledgerIds: ids, objective }
}
