...
export const RELEASE_PHASES = Object.freeze(['predeploy', 'deploy', 'smoke'])

export function normalizeQaTier(value, fallback = null) {
  const normalized = String(value || fallback || '').trim().toLowerCase()
  if (!Object.hasOwn(QA_TIERS, normalized)) {
    throw new Error('QA tier must be one of q0, q1, q2, or q3')
  }
  return normalized
}

export function normalizeQaProfile(value = 'standard', tier = 'q1') {
  const normalized = String(value || 'standard').trim().toLowerCase()
  const normalizedTier = normalizeQaTier(tier)
  if (!Object.hasOwn(QA_PROFILES, normalized)) {
    throw new Error('QA profile must be standard or deploy-fast')
  }
  if (normalized === 'deploy-fast' && normalizedTier !== 'q1') {
    throw new Error('deploy-fast QA profile is only valid with q1')
  }
  return normalized
}

export function qaPolicy({ tier, profile = 'standard' }) {
  const normalizedTier = normalizeQaTier(tier)
  const normalizedProfile = normalizeQaProfile(profile, normalizedTier)
  if (normalizedProfile === 'deploy-fast') {
  ...
  return Object.freeze({ ...QA_TIERS[normalizedTier], releaseRequired: false })
}
...
export async function gitHeadSha(cwd) {
  const result = await execFileAsync('git', ['-C', cwd, 'rev-parse', '--verify', 'HEAD'], {
    encoding: 'utf8',
    maxBuffer: MAX_JSON_BYTES,
  })
  const sha = result.stdout.trim().toLowerCase()
  if (!SAFE_GIT_SHA.test(sha)) throw new Error('Git HEAD is not a valid commit SHA')
  return sha
}

export async function gitRemoteBranchSha(cwd, remote, branch) {
  const normalizedRemote = String(remote || '')
  const normalizedBranch = String(branch || '')
  if (!SAFE_REMOTE.test(normalizedRemote)) throw new Error('Git remote name is invalid')
  if (
    !normalizedBranch
    || normalizedBranch.length > 1024
    || normalizedBranch.startsWith('-')
    || normalizedBranch.includes('\0')
  ) throw new Error('Git branch name is invalid')
  const git = async (args) => execFileAsync('git', ['-C', cwd, ...args], {
    encoding: 'utf8',
    maxBuffer: 8 * MAX_JSON_BYTES,
    timeout: 10_000,
    env: {
  ...
  try {
    await git(['check-ref-format', '--branch', normalizedBranch])
    await git(['remote', 'get-url', normalizedRemote])
    const reference = `refs/heads/${normalizedBranch}`
    const result = await git(['ls-remote', '--exit-code', '--refs', normalizedRemote, reference])
    const matches = result.stdout.trim().split(/\r?\n/).filter(Boolean)
      .map((line) => line.split(/\s+/))
      .filter((parts) => parts.length >= 2 && parts[1] === reference)
    const sha = String(matches[0]?.[0] || '').toLowerCase()
    if (matches.length !== 1 || !SAFE_GIT_SHA.test(sha)) throw new Error('invalid remote ref')
    return sha
  } catch {
    throw new Error('Named remote branch could not be verified')
  }
}
...
function hasSafeEvidenceCommand(command) {
  return Array.isArray(command)
    && command.length > 0
    && command.length <= MAX_EVIDENCE_COMMAND_PARTS
    && command.every((part) => (
      typeof part === 'string'
      && part.length > 0
      && part.length <= 4096
...
function hasSafeReleaseTarget(target) {
...
export async function validateReleaseProofPath({
  proofPath,
  phase,
  cwd,
  dataDir,
  ...
  try {
    ...
    if (!RELEASE_PHASES.includes(normalizedPhase)) {
      return { valid: false, reason: 'release proof has an invalid phase' }
    }
    const resolvedProofPath = path.resolve(proofPath)
    const runsRoot = path.join(path.resolve(dataDir), 'runs')
    if (!isInside(runsRoot, resolvedProofPath)) {
      return { valid: false, reason: 'release proof is outside the runs directory' }
    }
    const proof = await readContainedBoundedJson(runsRoot, resolvedProofPath)
    if (
      !SAFE_IDENTIFIER.test(String(proof.runId || ''))
      || (expectedRunId && proof.runId !== expectedRunId)
      || proof.phase !== normalizedPhase
    ) return { valid: false, reason: 'release proof has an invalid or mismatched identity' }

    const expectedPhaseDir = path.join(
      runsRoot,
      String(proof.runId || ''),
      'release',
      normalizedPhase,
    )
    if (resolvedProofPath !== path.join(expectedPhaseDir, 'proof.json')) {
      return { valid: false, reason: 'release proof path does not match its identity' }
    }
    if (
      proof.stdoutPath !== path.join(expectedPhaseDir, 'stdout.log')
      || proof.stderrPath !== path.join(expectedPhaseDir, 'stderr.log')
    ) return { valid: false, reason: 'release log paths do not match the private phase directory' }
    await assertContainedPath(runsRoot, proof.stdoutPath, { type: 'file' })
    await assertContainedPath(runsRoot, proof.stderrPath, { type: 'file' })

    if (
      proof.schemaVersion !== 1
      || proof.status !== 'passed'
      || proof.qaProfile !== 'deploy-fast'
      || proof.exitCode !== 0
      || proof.signal !== null
      || proof.timedOut !== false
      || proof.outputLimitExceeded !== false
      || !hasSafeEvidenceCommand(proof.command)
      || proof.workspaceDigestStarted !== expectedDigest
      || proof.workspaceDigest !== expectedDigest
      || proof.workspaceChanged !== false
      || proof.gitSha !== expectedGitSha
      || !SAFE_GIT_SHA.test(String(proof.gitSha || ''))
      || !hasValidEvidenceWindow(proof)
    ) return { valid: false, reason: 'release proof failed, changed, or is stale' }
    ...
    const runCwd = await realpath(run.cwd).catch(() => path.resolve(run.cwd || ''))
    const resolvedCwd = await realpath(cwd).catch(() => path.resolve(cwd))
    const runTier = normalizeQaTier(run.qaTier || 'q1')
    const runProfile = normalizeQaProfile(run.qaProfile || 'standard', runTier)
    if (
      run.runId !== proof.runId
      || runCwd !== resolvedCwd
      || runTier !== 'q1'
      || runProfile !== 'deploy-fast'
      || run.controller?.model !== 'gpt-5.6-sol'
    ) return { valid: false, reason: 'release proof belongs to another workspace or QA profile' }

    if (
      !SAFE_REMOTE.test(String(proof.remote || ''))
      || typeof proof.branch !== 'string'
      || proof.branch.length === 0
      || proof.branch.length > 1024
      || proof.branch.startsWith('-')
      || proof.branch.includes('\0')
      || proof.localSha !== proof.gitSha
      || proof.remoteShaBefore !== proof.gitSha
      || proof.remoteSha !== proof.gitSha
    ) return { valid: false, reason: 'release proof does not bind the local and remote SHA' }
    if (normalizedPhase === 'predeploy') {
      if (proof.shaBound !== true || !proof.command.includes(proof.gitSha)) {
        return { valid: false, reason: 'predeploy command is not bound to the exact Git SHA' }
      }
    } else if (
      !hasSafeReleaseTarget(proof.target)
      || (expectedTarget !== null && proof.target !== expectedTarget)
    ) return { valid: false, reason: 'release target is invalid or changed between phases' }

    return { valid: true, proof }
  } catch (error) {
    return { valid: false, reason: `release proof could not be validated: ${error?.message || 'unknown error'}` }
  }
}
...
export async function validateReleaseEvidence({
  ...
  try {
    const currentGitSha = await gitHeadSha(cwd)
    const predeploy = await validateReleaseProofPath({
      proofPath: release?.predeploy,
      phase: 'predeploy',
      cwd,
      dataDir,
      expectedDigest,
    ...
    if (!predeploy.valid) return predeploy
    const currentRemoteSha = await gitRemoteBranchSha(
      cwd,
      predeploy.proof.remote,
      predeploy.proof.branch,
    )
    if (currentRemoteSha !== currentGitSha) {
      return { valid: false, reason: 'The named remote branch no longer matches the release Git SHA.' }
    }
    const deploy = await validateReleaseProofPath({
      proofPath: release?.deploy,
      phase: 'deploy',
      cwd,
      dataDir,
      expectedDigest,
    ...
    if (!deploy.valid) return deploy
    const smoke = await validateReleaseProofPath({
      proofPath: release?.smoke,
      phase: 'smoke',
      cwd,
      dataDir,
      expectedDigest,
      expectedGitSha: currentGitSha,
      expectedRunId,
      expectedTarget: deploy.proof.target,
    })
    if (!smoke.valid) return smoke
    if (
      deploy.proof.remote !== predeploy.proof.remote
      || smoke.proof.remote !== predeploy.proof.remote
      || deploy.proof.branch !== predeploy.proof.branch
      || smoke.proof.branch !== predeploy.proof.branch
    ) return { valid: false, reason: 'Named remote or branch changed between release phases.' }

    const predeployStartedAt = Date.parse(predeploy.proof.startedAt)
    const predeployCompletedAt = Date.parse(predeploy.proof.completedAt)
    const deployStartedAt = Date.parse(deploy.proof.startedAt)
    const deployCompletedAt = Date.parse(deploy.proof.completedAt)
    const smokeStartedAt = Date.parse(smoke.proof.startedAt)
    if (predeployStartedAt <= latestWriterAt) {
      return { valid: false, reason: 'Predeploy evidence did not start strictly after write workers finished.' }
    }
    if (deployStartedAt <= predeployCompletedAt) {
      return { valid: false, reason: 'Deploy evidence did not start strictly after predeploy finished.' }
    }
    if (smokeStartedAt <= deployCompletedAt) {
      return { valid: false, reason: 'Smoke evidence did not start strictly after deploy finished.' }
    }
    if (release?.target !== deploy.proof.target || smoke.proof.target !== deploy.proof.target) {
      return { valid: false, reason: 'Release target changed between deploy and smoke evidence.' }
    }
    return {
      valid: true,
      gitSha: currentGitSha,
      target: deploy.proof.target,
      proofs: {
        predeploy: predeploy.proof,
        deploy: deploy.proof,
        smoke: smoke.proof,
      },
    }
  } catch (error) {
    return { valid: false, reason: error?.message || 'Release evidence validation failed.' }
  }
}
...
export async function validateClosure({ cwd, dataDir }) {
  try {
    ...
    if (
      ...
      return { valid: false, reason: 'The closure run does not match this workspace, QA tier, or QA profile.' }
    }
    ...
    const tierPolicy = qaPolicy({ tier, profile })
    if (tierPolicy.releaseRequired) {
      if (
        !Array.isArray(closure.tests)
        || closure.tests.length !== 0
        || !Array.isArray(closure.reviews)
        || closure.reviews.length !== 0
      ) return { valid: false, reason: 'Deploy-fast closure must use release evidence instead of local test or reviewer proof.' }
      ...
      if (!releaseResult.valid) return { valid: false, reason: releaseResult.reason }
      return { valid: true, closure, closurePath, ledger, currentDigest, release: releaseResult }
    }
