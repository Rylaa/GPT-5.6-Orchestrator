...
An installed controller derives the same plugin-managed data root that Codex passes to plugin hooks. `GPT56_ORCHESTRATOR_DATA_DIR` is the explicit standalone/test override. `dashboard` renders novice-readable live agent cards for exact-pinned external workers, with each worker's identity, model, effort, access, assigned task, current step, elapsed time, recent work, and report state. It derives bounded safe descriptions from Codex events and never exposes raw commands, command output, full prompts, credentials, or hidden reasoning. Its high-contrast basic ANSI palette distinguishes Sol authority, model families, lifecycle states, live activity, reports, and dim metadata; color is automatic in tmux and interactive terminals. It compacts cards to the live terminal dimensions and safely redraws reused rows so narrow panes remain readable. `NO_COLOR=1` keeps ordinary dashboard output plain, while the plugin-specific `GPT56_ORCHESTRATOR_COLOR=0` disables automatic tmux color and takes precedence for explicit control. Watched dashboards exit when all workers are terminal unless `--keep-open` is supplied. When Codex CLI is already inside an attached tmux client, the first worker `spawn` automatically opens one detached right-side live-work panel for the run; concurrent and later workers appear as additional cards in that panel. The pane closes automatically when all workers in the run are terminal, and new activity reopens it. Set `GPT56_ORCHESTRATOR_AUTO_PANE=0` to opt out. `pane` remains the manual recovery command. This is not a native Codex sidebar.
